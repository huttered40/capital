...
class rect{
public:
  rect(MPI_Comm comm, int64_t c, int64_t num_chunks=0){
    ...
    int64_t SubCubeSize = c*c*c;
    int64_t SubCubeSliceSize = c*c;
    MPI_Comm_split(comm, rank/SubCubeSize, rank, &this->cube);
    MPI_Comm_rank(this->cube, &cubeRank);
    int64_t temp1 = (cubeRank%c) + c*(cubeRank/SubCubeSliceSize);
    int64_t temp2 = rank % SubCubeSliceSize;
  ...
  int64_t c,d,x,y,z,num_chunks;
};

class square{
public:
  square(MPI_Comm comm, int64_t c, int64_t num_chunks=0){
    ...
    int64_t TopFaceSize = this->d*this->d;
    this->z = rank%c;
